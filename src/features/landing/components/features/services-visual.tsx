    ...
    <FeatureFrame label="Procedimientos">
      <div className="grid gap-2 sm:grid-cols-2">
        {SERVICE_ITEMS.map((service) => (
          <div
            key={service.name}
            className="rounded-lg border border-ink-800/4 bg-ink-150/60 p-3"
          >
            <div className="flex items-center justify-between">
              <span className={`rounded-md px-2 py-0.5 text-[10px] font-medium ${service.tone}`}>
                {service.dur}
              </span>
              <span className="text-xs font-semibold text-ink-950">{service.price}</span>
            </div>
            <p className="mt-2 text-xs font-medium text-ink-900">{service.name}</p>
          </div>
        ))}
      </div>
    </FeatureFrame>
