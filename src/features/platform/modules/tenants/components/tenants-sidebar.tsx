...
type Props = {
  search: string;
  status: TTenantStatusFilter;
  onSearchChange: (value: string) => void;
  onStatusChange: (value: TTenantStatusFilter) => void;
  onCreateClick?: () => void;
  canCreate?: boolean;
};

export const TenantsSidebar = ({
  search,
  status,
  onSearchChange,
  onStatusChange,
        ...
        {canCreate && onCreateClick && (
          <div className="mb-5 px-4">
            ...
            >
              ...
              <span className="text-sm">Crear Business</span>
            </Button>
          </div>
        )}
        ...
        <FormProvider {...methods}>
          <div className="px-4">
            <CustomFormSelect
              ...
              label="Estado"
              placeholder="Todos"
              items={TENANT_STATUS_FILTER_OPTIONS.map((option) => ({
                name: option.label,
                value: option.value,
              }))}
              onChange={(value) => {
                if (value == null) return;
                onStatusChange(value as TTenantStatusFilter);
              }}
            />
          </div>
        </FormProvider>
