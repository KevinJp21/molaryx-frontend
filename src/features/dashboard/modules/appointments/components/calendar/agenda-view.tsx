...
export const AgendaView = ({
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto rounded-2xl bg-ink-50 shadow-sm ring-1 ring-ink-300/60">
        ...
        <div className="mx-auto w-full max-w-3xl px-4 pb-10 md:px-6">
          {groupedEvents.map((group) => (
            <div key={group.date.toISOString()} className="relative">
              <div className="sticky top-0 z-10 border-b border-ink-200 bg-ink-50/95 py-4 backdrop-blur-md">
              ...
              <div className="space-y-3 py-4">
                {group.events.map((event) => (
                  <div
                    ...
                    className="group relative flex cursor-pointer gap-4 rounded-2xl border border-ink-200 bg-ink-50 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-accent-300 hover:shadow-lg hover:shadow-accent-500/5"
                  >
                    <span
                    ...
                    <div className="flex min-w-19 flex-col items-center pl-2">
                      ...
                      <span className="text-xs uppercase text-ink-600">
                        {format(event.start, "a", { locale: es }).replace(/\./g, "")}
                      </span>
                      <span className="my-1 h-3 w-px bg-ink-300" />
                      <span className="text-xs font-medium text-ink-600/80">
                    ...
                    <div className="min-w-0 flex-1 space-y-2">
                      <div className="flex items-start justify-between gap-2">
                        <h4 className="line-clamp-1 text-base font-semibold text-ink-950 transition-colors group-hover:text-accent-600">
                    ...
                    <ChevronRight className="absolute right-4 top-1/2 size-5 -translate-y-1/2 text-ink-600 opacity-0 transition-opacity group-hover:opacity-100" />
                  </div>
                ))}
              </div>
            </div>
